...
internal sealed class AToggleMissileBay : CardAction
{
    public override void Begin(G g, State s, Combat c)
    {
        ...
        if (!anyIsActive)
        {
            int cannonX = s.ship.parts.FindIndex((Part p) => p.key != null && validSiriusParts.Contains(p.key) && !p.active);
	...
	private AToggleMissileBay.Res? GetNextEnabledPartX(State s, Combat c)
	{
        ...
        List<int> partSlots = target.parts
            .AsValueEnumerable()
            .Select((Part p, int i) => new
            {
                part = p,
                x = i
            })
		...
		int nextEnabledPartSlot = (partSlots.FindIndex(x => x == currentPartX) + 1) % partSlots.Count;
    ...
    public override List<Tooltip> GetTooltips(State s)
    {
        ...
        return [
            new GlossaryTooltip($"{ModEntry.Instance.Package.Manifest.UniqueName}::ToggleMissileBayIcon")
            {
                Title = ModEntry.Instance.Localizations.Localize(["ship", "Sirius", "icon", "toggleMissileBay",  "name"]),
                Description = ModEntry.Instance.Localizations.Localize(["ship", "Sirius", "icon", "toggleMissileBay", "description"]),
    ...
    public override Icon? GetIcon(State s) => new Icon(Sprites.sirius_toggle.Sprite, null, Colors.white);
