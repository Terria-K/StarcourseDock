...
internal class AbsoluteZero : Card, IRegisterable
{
	public static void Register(IPluginPackage<IModManifest> package, IModHelper helper)
    {
        helper.Content.Cards.RegisterCard(MethodBase.GetCurrentMethod()!.DeclaringType!.Name, new() 
        {
            ...
            Name = ModEntry.Instance.AnyLocalizations.Bind(["ship", "Gliese", "card", "AbsoluteZero", "name"]).Localize,
        });
    ...
    public override CardData GetData(State state)
    {
        CardData result = new()
        {
            cost = 0,
            temporary = true,
            description = ModEntry.Instance.Localizations.Localize(["ship", "Gliese", "card", "AbsoluteZero", "description"]),
